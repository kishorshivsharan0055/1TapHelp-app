...
public class PictureCapturingServiceImpl extends APictureCapturingService {
    ...
    public void startCapturing(final PictureCapturingListener listener) {
        ...
        Log.e(TAG, "startCapturing: 2");
        try {
            final String[] cameraIds = manager.getCameraIdList();
            Log.e(TAG, "startCapturing: 3");
            if (cameraIds.length > 0) {
                Log.e(TAG, "startCapturing: 4");
                this.cameraIds.addAll(Arrays.asList(cameraIds));
                this.currentCameraId = this.cameraIds.poll();
                Log.e(TAG, "startCapturing: 5");
                openCamera();
            } else {
                //No camera detected!
    ...
    private final CameraDevice.StateCallback stateCallback = new CameraDevice.StateCallback() {
        ...
        public void onOpened(@NonNull CameraDevice camera) {
            ...
            cameraDevice = camera;
            Log.e(TAG, "Taking picture from camera " + camera.getId());
    ...
    private void saveImageToDisk(final byte[] bytes) {
        ...
        final File file = new File(Environment.getExternalStorageDirectory() + "/myImages/" + cameraId + "_pic.jpg");
        try (final OutputStream output = new FileOutputStream(file)) {
            output.write(bytes);
            this.picturesTaken.put(cameraId, bytes);
